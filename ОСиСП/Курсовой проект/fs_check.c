...
int check_all() {
    ...
    if (bsErrors > 0) {
        printf("При проверке загрузочного сектора найдено %d ошибок. Дальнейшая проверка файловой системы невозможна\n", bsErrors);
        ...
        if (input_int(1, 2) == 1) {               // Если пользователь выбирает попытку восстановления,
            ...
            if (copyOffset == -1) {   // Если не получилось найти копию загрузочного сектора
                printf("Не удалось обнаружить копию загрузочного сектора\n");
                return bsErrors;
            }
    ...
    if (fatErrors > 0) {
        ...
        if (input_int(1, 2) == 1) {                 // Если пользователь выбирает попытку восстановления
            if (!try_find_fat_copy()) {             // Если не получилось найти копию FAT-таблицы
                printf("Не удалось найти резервную копию основной FAT-таблицы\n");
                return fatErrors;
            }
...
int check_boot_sector(bool print)
{
    ...
    if (bs.checkSignature != 0xAA55) {
        ...
        printf("2 - Продолжить выполнение программы\n");
        if (input_int(1, 2) == 1) {
...
int check_fat_table(bool print) {
    ...
    for (uint32_t i = 2; i < entCount; i += 1) {
        ...
        for (uint32_t j = i; (fat[j] & 0x0FFFFFFF) != 0x0FFFFFFF; j = (fat[j] & 0x0FFFFFFF)) {
            ...
            if ((fat[j] & 0x0FFFFFFF) == 0 || (fat[j] & 0x0FFFFFFF) > entCount) { // Если следующий кластер пустой или за таблицей FAT, то разрыв
                if (print) {
                    ...
                    if ((fat[j] & 0x0FFFFFFF) == 0)
                        printf("Кластер %u указывает на пустой кластер (значение 0)\n", j);
                    else
...
bool try_find_fat_copy() {
    ...
    for (int i = 0; i < 8; i += 1) {
        ...
        fs_read(get_fatOffset(i, &bs), bs.fatSz_32, fat);    // Считываем (предполагаемую) таблицу FAT в буфер fat (размер fat не меняем, т.к. boot sector правильный), не read_fat_table(), чтобы без лишнего выделения паямяти
